...
class LlamaSession:
    def __init__(self, model="gpt-4.1", api_key=None, enable_web_search=True, enable_db_access=True):
        self.model = model
        self.api_key = "open-ai-api-key"
        ...
        if self.enable_db_access:
            self.db_agent = DBAgent(model=model, api_key=self.api_key)
    ...
    def web_agent(self, user_msg):
        ...
        try:
            print(f"[web_agent] Calling OpenAI API...")
    ...
    def user_assistant(self, user_msg_with_context):
        ...
        try:
            print("[user_assistant] Calling OpenAI API...")
            response = self.client.chat.completions.create(
                model=self.model,
